...
SQLALCHEMY_DATABASE_URL = 'sqlite:///C:\\Workspace\\Qriocity\\Other Domain Projects\\Apache Superset_Without_venv\\stock_data.db'
...
with engine.begin() as conn:
    ...
    print("🧹 Cleared existing stock_data.")
...
def fetch_financials(symbol):
    ...
    try:
        ...
        return {'Dividend': 0, 'EPS': 0, 'PE Ratio': 0, 'Beta': 0}
...
def calculate_additional_metrics(data, symbol):
    try:
        ...
        print(f"[{symbol}] Columns in downloaded data: {df.columns.tolist()}")
        ...
        # Use Close as substitute for Adj Close
...
def fetch_and_store_stock_data(symbols):
    for symbol in symbols:
        try:
            # Download data
            data = yf.download(symbol, period="3mo", interval="1d", auto_adjust=False)
            print(data.tail())
